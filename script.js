...
select = document.querySelectorAll(".selt");
const countryList = {
    AED: "AE",
    AFN: "AF",
    XCD: "AG",
...
msg = document.querySelector('.msg p')
...
for (selet of select){
   for (currcode in countryList){
       ...
       optn.value = currcode;
       if (selet.name === 'from' && currcode === "USD"){
        optn.selected = 'selected';
       }else if (selet.name === 'to' && currcode === 'INR'){
        optn.selected = 'selected';
       }
       selet.append(optn);
    }
...
const updateRate = async () =>{
    amt = document.querySelector('input')
    ...
    let final_rate = amt.value * rate;
    msg.innerText = `${amt.value} ${fromCurr.value} = ${final_rate} ${toCurr.value}`;
}
